...
export const createProperty = async (req, res) => {
  try {
    ...
    // - normal users use req.user.company
    // - system admins may pass active company from frontend as req.body.business
    const businessId =
      req.user?.company ||
      business;
    ...
    // Filter and validate landlords - only include entries with name + landlordId
    ...
    // Filter and validate standing charges
    ...
    // Filter and validate security deposits
    ...
    const property = new Property({
      ...
      roadStreet,
      zoneRegion,
      address: address || `${roadStreet || ""}, ${estateArea || ""}, ${townCityState || ""}`.replace(/^,\s*|,\s*$/g, "").replace(/,\s*,/g, ","),
      accountLedgerType,
      primaryBank,
      alternativeTaxPin,
      invoicePrefix,
...
export const bulkImportProperties = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(properties) || properties.length === 0) {
      return res.status(400).json({ success: false, message: "Properties array is required" });
    }

    if (properties.length > 1000) {
      return res.status(400).json({ success: false, message: "Maximum 1000 properties per import" });
    }

    const businessId = req.user?.company || business;

    if (!businessId) {
      return res.status(400).json({ success: false, message: "Business/Company ID is required" });
    }

    const lrNumbers = properties.filter((p) => p.lrNumber).map((p) => p.lrNumber);
    const providedCodes = properties.filter((p) => p.propertyCode).map((p) => p.propertyCode);
    ...
    for (const property of properties) {
      ...
      try {
        ...
        if (!generatedPropertyCode) {
          ...
          while (existingPropertyCodes.has(generatedPropertyCode) || seenCodesInBatch.has(generatedPropertyCode)) {
            counter++;
            generatedPropertyCode = `PRO${String(counter).padStart(3, "0")}`;
          }
