...
export const closeStatement = async (req, res) => {
  try {
    ...
    const newStatement = new ProcessedStatement({
      ...
      periodStart: new Date(periodStart),
      periodEnd: new Date(periodEnd),
      statementType: statementType || "provisional",
      totalRentInvoiced: Number(totalRentInvoiced) || 0,
      totalRentReceived: Number(totalRentReceived) || 0,
      totalRentReceivedByManager: Number(totalRentReceivedByManager) || 0,
      totalRentReceivedByLandlord: Number(totalRentReceivedByLandlord) || 0,
      totalUtilitiesCollected: Number(totalUtilitiesCollected) || 0,
      depositsHeldByManager: Number(depositsHeldByManager) || 0,
      depositsHeldByLandlord: Number(depositsHeldByLandlord) || 0,
      unappliedPayments: Number(unappliedPayments) || 0,
      ...
      recurringDeductions: Number(recurringDeductions) || 0,
      advanceRecoveries: Number(advanceRecoveries) || 0,
      expensesByCategory: expensesByCategory || {},
      netAfterExpenses: Number(netAfterExpenses) || Number(netAmountDue) || 0,
      summaryBuckets: summaryBuckets || {},
      financialEvents: Array.isArray(financialEvents) ? financialEvents : [],
      isNegativeStatement: !!isNegativeStatement,
      amountPayableByLandlordToManager: Number(amountPayableByLandlordToManager) || 0,
